...
const securityLayers = [
  {
    ...
    color: '#7C3D0F',
    glow: 'rgba(124, 61, 15,0.3)',
    bg: 'bg-[#7C3D0F]/10',
    border: 'border-[#7C3D0F]/25',
...
const stats = [
  { value: '256-bit', label: 'AES Encryption', color: '#7C3D0F' },
  { value: '99.9%', label: 'Uptime SLA Target', color: '#8B4513' },
  { value: '0', label: 'Data Breaches Ever', color: '#166534' },
  { value: '24/7', label: 'Security Monitoring', color: '#7C3D0F' },
]

const compliance = [
  { name: 'GDPR Compliant', desc: 'Full EU data protection compliance', icon: '🇪🇺', color: '#7C3D0F' },
  { name: 'ISO 27001 Target', desc: 'Information security management', icon: '🔒', color: '#8B4513' },
  { name: 'SOC 2 Type II', desc: 'Service organization controls', icon: '✅', color: '#166534' },
  { name: 'PCI DSS Ready', desc: 'Payment card data security', icon: '💳', color: '#7C3D0F' },
]
    ...
    >
      <div className="flex flex-col items-center">
        ...
        >
          ...
          <span
            className="absolute -top-2 -right-2 w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-black text-[#1A1410]"
      ...
      <div className={isLast ? 'pb-0' : 'pb-10 lg:pb-14'}>
        ...
        <h3 className="text-xl lg:text-2xl font-black text-[#1A1410] mb-3">{layer.title}</h3>
        <p className="text-[#1A1410]/55 leading-relaxed text-[15px]">{layer.desc}</p>
      </div>
    </motion.div>
  )
}
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative overflow-hidden pt-32 pb-16">
        ...
        <div className="absolute top-24 right-1/4 w-[400px] h-[400px] bg-[#7C3D0F]/4 rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute top-24 left-1/4 w-[300px] h-[300px] bg-[#8B4513]/4 rounded-full blur-[100px] pointer-events-none" />
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#7C3D0F]/20 bg-[#7C3D0F]/5 mb-6">
            ...
            <h1 className="text-5xl font-black text-[#1A1410] sm:text-6xl lg:text-7xl mb-6">
              {t('security.title')}
            </h1>
            <p className="mx-auto max-w-2xl text-lg text-[#6B5D52]">
              {t('security.subtitle')}
            </p>
            <div className="mt-8 inline-flex items-center gap-3 px-5 py-3 rounded-2xl border border-[#7C3D0F]/20 bg-[#7C3D0F]/5">
              <AlertTriangle className="h-4 w-4 text-[#7C3D0F]" />
              <span className="text-sm font-semibold text-[#1A1410]">
                Your students' data is a <span className="text-[#7C3D0F]">legal responsibility</span>. REVENEX takes it seriously.
              </span>
            </div>
          </motion.div>
        </div>
      ...
      {/* Stats bar */}
      <section className="py-10 border-y border-[#E8E0D4] relative overflow-hidden">
        <div className="absolute inset-0 bg-[#7C3D0F]/2 pointer-events-none" />
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <section className="py-20 lg:py-28 relative">
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 40% 60% at 0% 50%, rgba(124, 61, 15,0.04), transparent)' }} />
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          >
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#E8E0D4] bg-[#F0E8DC] mb-5">
                <span className="text-xs font-black text-[#3D3128] uppercase tracking-widest">Architecture</span>
              </div>
              <h2 className="text-4xl font-black text-[#1A1410] sm:text-5xl leading-tight mb-4">
                6-Layer Security<br /><span className="gradient-text">No Weak Points.</span>
              </h2>
            </div>
            <div>
              <p className="text-[#3D3128] leading-relaxed">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#E8E0D4] bg-[#F0E8DC] mb-4">
              <span className="text-xs font-black text-[#3D3128] uppercase tracking-widest">Compliance</span>
            </div>
            <h2 className="text-3xl font-black text-[#1A1410] sm:text-4xl">
              {language === 'en' ? 'Compliance & Certifications' : 'अनुपालन और प्रमाणपत्र'}
            </h2>
            <p className="text-[#6B5D52] mt-2 text-sm">
            ...
            {compliance.map((cert, i) => (
              ...
              >
                ...
                >
                  ...
                  <div className="font-bold text-[#1A1410] mb-1">{cert.name}</div>
                  <div className="text-xs text-[#6B5D52]">{cert.desc}</div>
                </TiltCard>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-16 lg:py-24">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <TiltCard className="glass-card animated-border rounded-3xl p-10 text-center border-[#7C3D0F]/20">
              <div className="mx-auto mb-6 w-16 h-16 gradient-bg rounded-2xl flex items-center justify-center shadow-[0_0_40px_rgba(124,61,15,0.3)]">
                <Award className="h-8 w-8 text-[#1A1410]" />
              </div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#7C3D0F]/20 bg-[#7C3D0F]/5 mb-5">
                <span className="text-xs font-black text-aqua/80 uppercase tracking-widest">Our Promise</span>
              </div>
              <h2 className="text-3xl font-black text-[#1A1410] mb-4">
                {language === 'en' ? 'We Will Never Sell Your Data' : 'हम आपका डेटा कभी नहीं बेचेंगे'}
              </h2>
              <p className="text-[#3D3128] leading-relaxed mb-8 max-w-xl mx-auto">
                {language === 'en'
              ...
              <div className="grid sm:grid-cols-3 gap-4 mb-8">
                {[
                  ...
                  <div key={item.label} className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-[#F0E8DC] border border-[#E8E0D4]">
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <Link href="/book-demo">
              <motion.span whileHover={{ scale: 1.03 }} className="inline-flex items-center gap-2 gradient-bg text-[#1A1410] font-bold px-8 py-4 rounded-2xl cursor-pointer shadow-[0_0_30px_rgba(124,61,15,0.25)]">
