import { useState } from 'react'
  ...
  export default function Login() {
    ...
    const inputClass = (err?: string) =>
      `w-full rounded-xl border ${err ? 'border-red-500/50 bg-red-500/5' : 'border-[#E8E0D4] bg-[#F0E8DC]'} pl-10 pr-4 py-3.5 text-[#1A1410] placeholder-[#6B5D52] outline-none focus:border-aqua/50 transition-all text-sm`
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-4 py-16 relative overflow-hidden">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[300px] h-[300px] bg-[#7C3D0F]/5 rounded-full blur-[120px] pointer-events-none" />
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
            ...
            <div className="p-8">
              <div className="text-center mb-8">
                <div className="mx-auto mb-4 w-14 h-14 gradient-bg rounded-2xl flex items-center justify-center">
                  <LogIn className="h-7 w-7 text-[#1A1410]" />
                </div>
                <h1 className="text-2xl font-black text-[#1A1410] mb-1">Welcome back</h1>
                <p className="text-sm text-[#6B5D52]">Sign in to your REVENEX account</p>
              </div>
              ...
              <div className="relative my-5">
                ...
                <div className="relative flex justify-center text-xs"><span className="px-3 bg-transparent text-[#6B5D52] font-medium">or continue with email</span></div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <label className="text-sm font-medium text-[#3D3128]">Password</label>
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6B5D52] hover:text-[#3D3128] transition-colors">
              ...
              <div className="mt-6 pt-6 border-t border-[#E8E0D4] text-center">
                <p className="text-sm text-[#6B5D52]">Don't have an account?{' '}
          ...
          <div className="mt-6 text-center">
            <Link href="/"><span className="text-xs text-[#6B5D52] hover:text-[#3D3128] transition-colors cursor-pointer">← Back to home</span></Link>
          </div>
        </motion.div>
                  ...
                  {forgotSuccess ? (
                    ...
                    <form onSubmit={handleForgotPassword} className="space-y-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="email" placeholder="your@email.com" value={forgotEmail}
                            ...
                            className="w-full rounded-xl border border-[#E8E0D4] bg-[#F0E8DC] pl-10 pr-4 py-3.5 text-[#1A1410] placeholder-[#6B5D52] outline-none focus:border-aqua/50 transition-all text-sm" />
                        </div>
                      ...
                      <motion.button type="submit" disabled={forgotLoading} whileHover={!forgotLoading ? { scale: 1.02 } : {}} whileTap={!forgotLoading ? { scale: 0.98 } : {}}
                        className="w-full gradient-bg text-[#1A1410] font-bold py-3.5 rounded-2xl flex items-center justify-center gap-2 text-sm disabled:opacity-60">
                        {forgotLoading ? (
