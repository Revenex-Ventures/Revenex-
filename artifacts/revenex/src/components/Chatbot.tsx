...
export function Chatbot() {
  ...
  return (
    <>
      <AnimatePresence>
        {!isOpen && (<motion.button initial={{ scale:0,opacity:0 }} animate={{ scale:1,opacity:1 }} exit={{ scale:0,opacity:0 }} transition={{ delay:1.5,type:'spring',stiffness:260,damping:20 }} onClick={() => setIsOpen(true)} className='fixed bottom-6 right-6 z-50' aria-label='Open chat'>
          <div className='relative w-14 h-14 gradient-bg rounded-2xl flex items-center justify-center shadow-2xl glow-aqua transition-transform hover:scale-110'>
            <MessageCircle className='h-6 w-6 text-[#1A1410]' />
      ...
      <AnimatePresence>
        {isOpen && (<motion.div initial={{ opacity:0,y:40,scale:0.95 }} animate={{ opacity:1,y:0,scale:1 }} exit={{ opacity:0,y:40,scale:0.95 }} transition={{ type:'spring',stiffness:300,damping:30 }} className='fixed bottom-6 right-6 z-50 w-[360px] sm:w-[400px] max-h-[600px] flex flex-col rounded-3xl overflow-hidden border border-[#E8E0D4] shadow-2xl' style={{ background:'linear-gradient(135deg,#0a0a1a 0%,#050816 100%)' }}>
          <div className='flex items-center justify-between px-5 py-4 border-b border-[#E8E0D4] gradient-bg'>
            <div className='flex items-center gap-3'><div className='w-9 h-9 rounded-xl bg-[#1A1410]/20 flex items-center justify-center'><Bot className='h-5 w-5 text-[#1A1410]' /></div>
              <div><p className='font-semibold text-[#1A1410] text-sm'>REVENEX Assistant</p>
                <div className='flex items-center gap-1'><span className='w-2 h-2 rounded-full inline-block' style={{ background: offline ? '#f87171' : '#22c55e' }} />
                  {offline ? <span className='text-[#1A1410] text-xs'>API server offline</span> : <><span className='text-[#1A1410] text-xs'>AI-powered</span><Sparkles className='h-3 w-3 text-[#3D3128]' /></>}</div></div></div>
            <button onClick={() => setIsOpen(false)} className='w-8 h-8 rounded-xl bg-[#1A1410]/20 flex items-center justify-center hover:bg-[#1A1410]/30' aria-label='Close'><X className='h-4 w-4 text-[#1A1410]' /></button>
          </div>
          ...
          <div className='flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-[280px] max-h-[380px]'>
            {messages.length === 0 && (<div className='text-center py-6'>
              <div className='w-14 h-14 gradient-bg rounded-2xl flex items-center justify-center mx-auto mb-3'><Bot className='h-7 w-7 text-[#1A1410]' /></div>
              <p className='text-[#1A1410]/80 font-medium text-sm'>{language==='en'?'Hi! How can I help you?':'नमस्ते!'}</p>
              <div className='mt-4 flex flex-wrap gap-2 justify-center'>
                {QUICK.map(q=>(<button key={q.en} onClick={()=>void send(language==='en'?q.en:q.hi)} className='text-xs px-3 py-1.5 rounded-full border border-[#E8E0D4] text-[#3D3128] hover:border-[#7C3D0F]/40 hover:text-[#7C3D0F] transition-colors'>{language==='en'?q.en:q.hi}</button>))}</div></div>)}
            {messages.map(msg=>(<motion.div key={msg.id} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className={'flex '+(msg.isUser?'justify-end':'justify-start')}>
              <div className={'max-w-[82%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed '+(msg.isUser?'gradient-bg text-[#1A1410] rounded-br-sm':msg.isError?'bg-red-500/10 border border-red-500/20 text-red-300 rounded-bl-sm':'bg-[#F0E8DC] border border-[#E8E0D4] text-[#1A1410]/90 rounded-bl-sm')}>{msg.text}
                {(!msg.isUser && msg.text.startsWith('Model:')) ? (<div className='text-[10px] text-[#3D3128] mt-1'>{msg.text}</div>) : null}
              </div>
            </motion.div>))}
            {typing&&(<motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className='flex justify-start'><div className='bg-[#F0E8DC] border border-[#E8E0D4] px-4 py-3 rounded-2xl rounded-bl-sm flex gap-1.5 items-center'>{[0,0.15,0.3].map((d,i)=>(<motion.span key={`dot-${i}`} className='w-2 h-2 rounded-full bg-[#7C3D0F]' animate={{y:[0,-5,0]}} transition={{repeat:Infinity,duration:0.8,delay:d}} />))}</div></motion.div>)}
            <div ref={endRef} />
          </div>
          <div className='px-4 pb-4 pt-2 border-t border-[#E8E0D4]'>
            <div className='flex gap-2 items-center bg-[#F0E8DC] border border-[#E8E0D4] rounded-2xl px-4 py-2'>
              <input type='text' value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();void send()}}} placeholder={language==='en'?'Type your message...':'अपना संदेश लिखें...'} className='flex-1 bg-transparent text-[#1A1410]/90 placeholder-white/30 text-sm outline-none' disabled={typing} />
              <button onClick={()=>void send()} disabled={!input.trim()||typing} className='w-8 h-8 gradient-bg rounded-xl flex items-center justify-center disabled:opacity-40 flex-shrink-0' aria-label='Send'><Send className='h-3.5 w-3.5 text-[#1A1410]' /></button>
            </div>
          </div>
        </motion.div>)}
      </AnimatePresence>
